...
除外した`lines`に対して、商品名の行(`lines[i]`)の**直後**に「→ 商品を
見る」の行(`lines[i + 1]`)が続くという**隣接2行の完全一致**であり、
部分一致・前方一致・「商品名だけの一致」は一切候補にしません。
...
その後、`ARTICLE-001`の実機HTMLダンプを直接解析した結果、隣接2行の判定
自体は正しいが、探索範囲が本文editor配下の**descendant(子孫)**まで
及んでいたことが真因と判明しました。本文editorの直接の子ではない、別の
階層にネストした要素の中にも同じ`[label, "→ 商品を見る"]`のパターンが
偶然存在し、通常のCSSセレクタ`locator("p")`(あらゆる深さの子孫にマッチ
する`document.querySelectorAll`と同じ挙動)がそれも候補として拾って
しまっていました。実機のdirect child構造を確認すると、商品導線は本文
editorの直接の子として独立した`<p>`で存在していたため、探索スコープを
`:scope > p`(直接の子のみ)に限定しました。これにより、内包関係にある
要素は最初から候補にすらならず、隣接2行判定・行構成の最終チェックは
そのままdirect childの候補だけに対して行われます。候補ブロックが1件以上
見つかった場合は
(一意特定・複数件での安全停止のいずれも)、そのブロックのindexと正規化
済みlinesを診断ログに記録し、次回の実機実行で問題が起きた場合も1回で
原因を切り分けられるようにしています。
