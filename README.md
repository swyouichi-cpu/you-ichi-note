...
商品名(1.)の一致判定は、`text.splitlines()`で行単位に分解した各行との
**完全一致**(`link.label in lines`。空行は`if line.strip()`で除外して
から比較)であり、部分一致・前方一致は一切行いません。`ARTICLE-001`の
実機実行(2026年8月29日)で、商品名の一方がもう一方の商品名の先頭部分と
一致するケース(「TOY JAM 瀬戸内レモン」と「TOY JAM 瀬戸内レモン月桂樹」)
で商品名を含むブロックが2件見つかったと誤判定される事象が判明したため、
この行単位の完全一致であることをテストで明示的に固定しました。
