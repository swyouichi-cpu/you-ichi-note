...
詳細な安全設計は「10. Phase 1の安全要件(絶対に維持する)」を参照してください。
...
本文入力欄(ProseMirror)を特定して本文を入力
  (本文末尾に5行改行 + "#タグ1 #タグ2 ..." を追記した状態で入力。
   タグ0件なら本文はそのまま)
   ↓
本文入力欄からread-back確認(入力した内容と一致するか)
   ↓
autosave完了を確認
   ↓
「下書き保存」のみ実行(公開系ボタンは一切押さない)
   ↓
保存後にもう一度、本文入力欄からread-back確認
   ↓
note_url取得
   ↓
...
| `craft_url` | CraftのURL/ID(Craft連携は未着手。「12. Phase 2以降の検討事項」参照) |
...
## 12. Phase 2以降の検討事項(Phase 1のスコープ外)

以下はPhase 1では意図的に対応していません。着手する場合は、Phase 1の
安全要件(前掲「10.」)を変更しない形で、別途合意のうえで進めます。
...
- 自動公開・予約投稿・公開設定パネルの自動操作: 「10. Phase 1の安全要件」で
  禁止している通り、これらはPhase 1はもちろん、今後のPhaseで追加する場合も
  安全設計自体を変更する重大な決定であるため、その都度明示的な合意が必要です。

なお、noteのUI変更で本文editorのセレクタが今後壊れる可能性はゼロではありません。
その場合は「5. 秘密情報を共有せずに動作確認する方法」の手順で診断データを取得し、
実機DOMに基づいて`_fill_body()`の候補セレクタを更新します(推測では追加しません)。
