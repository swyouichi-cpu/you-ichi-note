...
Pythonスクリプトがシートを確認
   ↓
対象記事(status=ready かつ publish_atが空または現在時刻以前)があれば
status: ready → processing
   ↓
Playwrightでnoteに下書き作成(公開はしない。タグは本文末尾に追記)
   ↓
成功したら status: processing → draft_created (note_urlを記録)
   ↓
...
- `needs_review`: 成功したか失敗したかシステムが確定できない状態。**自動では再処理されません。** タグの形式があいまいで自動判定できなかった場合(後述「8. noteのタグ設定について」)もこの状態になります。note側の実際の状態を確認し、手動で`ready`(下書きが存在しない場合)または`draft_created`(下書きが存在する場合、note_urlも手動で埋める)に書き換えてください。
...
`tags`列そのものは変更しません。タグが1件も無い場合は本文を一切変更しません。
...
**最終確認について**: 実際にnote側が本文末尾の`#タグ名`をハッシュタグとして
正しく認識するかどうかは、**人間が下書きを手動で開き「公開に進む」を押して
公開設定パネルの表示を目視することでのみ確認してください。この確認は
