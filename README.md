...
本文入力欄(ProseMirror)を特定して本文を入力
  (本文末尾に5行改行 + 商品導線セクション + 5行改行 + "#タグ1 #タグ2 ..."
   ...
   ↓
本文入力欄からread-back確認(テキスト)
   ↓
autosave完了を確認
   ↓
「下書き保存」のみ実行(公開系ボタンは一切押さない)
   ↓
保存後にもう一度、本文read-back確認
   ↓
note_url取得
   ↓
商品導線(product_links)が無い記事:
  Sheetsへ書き戻し(status=draft_created, note_url, updated_at)
商品導線(product_links)がある記事:
  商品リンクの自動設定は行わない(note.com側UIの実機での不安定さのため。
  詳細は「10. 商品リンク」)。Sheetsへ書き戻し
  (status=needs_review, note_url, updated_at, 手動リンク設定を促す
  error_message)
   ↓
...
一致すれば正常終了。不一致ならneeds_reviewへ倒す
   ↓
あなたが下書きを手動で開いて確認
  (商品導線がある記事はここでリンクも手動設定し、statusをdraft_createdへ)
  → 手動で公開ボタンを押す
```
...
- `needs_review`: 成功したか失敗したかシステムが確定できない状態。**自動では再処理されません。** 以下のいずれかで発生します。note側の実際の状態を確認し、手動で`ready`(下書きが存在しない場合)または`draft_created`(下書きが存在する場合、note_urlも手動で埋める)に書き換えてください。
  ...
  - `product_links`の形式が不正な場合(後述「10. 商品リンク(本文末尾のテキストリンク方式)」)
  ...
  **`needs_review`かつ`note_url`が入っている特別なケース(2026年8月29日〜)**:
  `product_links`が指定されている記事は、下書き作成自体(タイトル・本文・
  下書き保存)は**成功**しているものの、商品リンクの設定は自動化しておらず
  `needs_review`のまま`note_url`を保持します(`error_message`に「商品リンク
  の手動設定が必要です」という趣旨のメッセージが入ります)。この場合は
  「下書きが存在するか確認する」手順は不要です(既に存在することが
  分かっています)。note下書きを開き、本文中の商品名・「→ 商品を見る」へ
  手動でリンクを設定したうえで、`status`を`draft_created`に変更して
  ください(詳細は「10. 商品リンク(本文末尾のテキストリンク方式)」)。
...
**★現在の仕様(2026年8月29日、ARTICLE-001の実機実行を踏まえた運用方針の
変更。production運用はこの節が最新です)★**: note.comの商品リンク設定UI
(フローティング編集ツールバー・URL入力欄・「適用」ボタン)は、これまでの
一連の実機実行で繰り返し不安定さが見つかりました(URL入力欄の消失、
値の残留・連結、`position: fixed`によるviewport外配置、2商品目以降での
`<a>`反映timeoutなど)。これ以上の自動化追跡はコストに見合わないと判断し、
**productionでは商品リンクの自動設定を行いません**。本文にはECの生URLを
一切含めず(商品カード自動変換を避けるため)、`product_links`列から
「商品名」「→ 商品を見る」というプレーンテキストの商品導線セクションを
本文末尾へ自動生成しますが、実際のリンク設定(`<a>`要素の生成)は行わず、
商品名・「→ 商品を見る」はリンクの無い通常テキストのまま下書きに残ります。
`product_links`が指定された記事は、下書き作成・下書き保存自体は成功し
`note_url`も取得できますが、`status`は`draft_created`ではなく
`needs_review`(`note_url`保持)になります。人間がnote下書きを開き、
該当箇所へ手動でリンクを設定したうえで`status`を`draft_created`へ変更して
ください(詳細は「3. statusの状態遷移」)。

以下は、商品リンクを実際にnote上で自動設定しようとした際の設計・実機
検証の記録です。関連するメソッド(`_apply_product_links()`等)・例外
クラスはコードとしては削除せず残していますが、**production run
(`create_draft()`)からは呼び出していません**。将来note.com側のUIが
安定した場合の再検証に備えて残しているdead codeです。

**方式(過去の設計・現在は自動リンク設定部分のみ無効化)**: 本文には
ECの生URLを一切含めません。代わりに`product_links`列(下記フォーマット)
から、本文末尾に以下のようなプレーンテキストの商品導線セクションを追記
します。かつては、この「→ 商品を見る」という固定文言だけに、noteの
フローティング編集ツールバーの「リンク」ボタン(下記参照)経由でインライン
リンクを設定しようとしていました。2026年8月29日に「適用」クリック後の
実機Artifactで`<a>`要素の生成を確認できたため、いったんは下書き保存・
保存後read-back確認まで一連の流れとして実装しましたが、その後の複数商品
での実機実行で上記の不安定さが判明したため、上記の通りproductionでは
リンク設定部分を呼ばない運用に変更しました。
...
(`body_locator`、ProseMirrorの実DOM)の**直接の子**である`<p>`要素
(`body_locator.locator(":scope > p")`。descendant(子孫)まで拾う通常の
`locator("p")`ではない)にスコープを絞ったうえで、
...
商品名(1.)の一致判定は、`text.splitlines()`で行単位に分解し空行を
除外した`lines`が、`[label, "→ 商品を見る"]`という**ちょうど2行だけ**で
構成されている(`lines == [label, "→ 商品を見る"]`、ブロック全文の完全
一致)ことであり、部分一致・前方一致・「商品名だけの一致」・「ブロック内
のどこかに存在すれば良い」という判定は一切行いません。
`ARTICLE-001`の実機実行(2026年8月29日)で、まず商品名の一方がもう一方の
商品名の先頭部分と一致するケース(「TOY JAM 瀬戸内レモン」と「TOY JAM
瀬戸内レモン月桂樹」)で商品名を含むブロックが2件見つかったと誤判定される
事象が判明し、続けて別の実機実行で、本文中のふつうの文章にたまたま商品名
だけの行が出現した場合にも誤って候補に含まれうることが判明したため、
「商品名の行」単独ではなく「商品名の行→『→ 商品を見る』の行」という
隣接ペアの有無で候補を判定するように修正しました。一意に特定できた場合も
念のため、そのブロック全体の行構成がちょうど`[label, "→ 商品を見る"]`の
2行であることを改めて確認します。

その後、`ARTICLE-001`の実機HTMLダンプを直接解析した結果、探索範囲が
本文editor配下の**descendant(子孫)**まで及んでいたことが真因と判明
しました。本文editorの直接の子ではない、別の階層にネストした要素の中にも
同じ`[label, "→ 商品を見る"]`のパターンが偶然存在し、通常のCSSセレクタ
`locator("p")`(あらゆる深さの子孫にマッチする`document.querySelectorAll`
と同じ挙動)がそれも候補として拾ってしまっていました。実機のdirect child
構造を確認すると、商品導線は本文editorの直接の子として独立した`<p>`で
存在していたため、探索スコープを`:scope > p`(直接の子のみ)に限定
しました。

さらにその後の実機再実行で、`ARTICLE-001`の実際の本文editorには、
記事本文ほぼ全体を`inner_text()`として含む巨大なdirect-child `<p>`と、
商品導線専用のdirect-child `<p>`の両方が存在し、巨大な`<p>`の`lines`の
**途中**にも`[label, "→ 商品を見る"]`という2行が偶然含まれていたことが
判明しました。「ブロック内のどこかに該当の2行が存在すれば候補」という
判定では、この巨大なブロックも候補になってしまうため、候補条件を
「ブロックの`lines`全体がちょうどその2行だけで構成されているか」という
全文完全一致に厳密化しました(前述の通り)。これにより、記事本文の一部と
してたまたま同じ2行を内部に含む巨大な`<p>`は、他にも本文の地の文を含み
`lines`がちょうど2行にはならないため候補になりません。候補ブロックが
1件以上見つかった場合は(一意特定・複数件での安全停止のいずれも)、その
ブロックのindexと正規化済みlinesを診断ログに記録し、次回の実機実行で
問題が起きた場合も1回で原因を切り分けられるようにしています。
...
12-a. **(2026年8月29日追加)** `product_links`が指定されている記事は、
    商品リンクの自動設定(下記13〜22)を`create_draft()`から呼ばない。
    下書き作成・下書き保存自体が成功した場合は、`draft_created`ではなく
    `needs_review`(`note_url`保持)として人間による手動リンク設定を促す
    (`mark_needs_review_with_note_url()`、詳細は「10. 商品リンク」「3.
    statusの状態遷移」)。この行が自動処理で再取得され新しいnote下書きが
    重複作成されることはない(`get_next_target_article()`は
    `status=ready`の行のみを対象にするため)。

★以下13〜22は、商品リンクを実際にnote上で自動設定しようとした際の設計・
実機検証の記録です。関連するメソッド・例外クラスはコードとしては削除して
いませんが、**productionの`create_draft()`からは呼び出していません**
(dead code。将来note.com側のUIが安定した場合の再検証に備えて残して
います)。呼び出されない状態であっても、これらのメソッドが将来再度
呼び出される際に安全であるべき設計方針として、以下は変更・緩和しません。
...
| `ARTICLE-001`(商品名prefix誤判定) | 完成実装後、2商品(片方がもう片方の商品名のprefix)での実機実行 | `needs_review`(商品名を含むブロックが2件と誤判定。行単位の完全一致へ修正。対応不要) |
...
| `ARTICLE-001`(commit d640224での再実行、運用方針変更) | fill()導入後の実機再実行 | 2商品目のURL入力・「適用」までは到達したが`_wait_for_product_link_applied()`が3000ms以内に反映を確認できず`needs_review`。これ以上の自動化追跡はコストに見合わないと判断し、productionでは商品リンク自動設定を呼ばず、`needs_review`(`note_url`保持)で人間に手動設定を委ねる運用へ変更。対応不要(ARTICLE-001は未変更) |
...
`ARTICLE-001`(商品名prefix誤判定)で判明した事項: 完成実装後、
`product_links`に2商品(「TOY JAM 瀬戸内レモン」「TOY JAM 瀬戸内レモン
月桂樹」)がある状態で実機実行したところ、「商品名『TOY JAM 瀬戸内
レモン』を含むブロックが本文editor内に2件見つかりました」として
`needs_review`へ安全停止した。一方の商品名がもう一方の商品名の先頭部分と
一致するケースを`_find_product_link_block()`の商品名判定が誤って両方の
ブロックにマッチさせてしまったことが原因と判明したため、商品名の判定を
`text.splitlines()`で行単位に分解した各行との完全一致
(`link.label in lines`。空行は除外)に基づくものであることを改めて
明示的に固定し、2商品(片方がもう片方の商品名のprefix)がそれぞれ正しい
ブロックへ解決されることをテストで確認した(詳細は「10. 商品リンク」)。
この修正もローカルpytestでの確認のみで、まだ実際のGitHub Actions実行
では検証していない。

`ARTICLE-001`(隣接2行判定への再修正)で判明した事項: 上記の行単位の
完全一致への修正(`link.label in lines`)は、リストの要素との等価判定で
あり部分一致ではないが、「商品名の行が存在するか」だけを候補条件にして
いるため、本文中のふつうの文章にたまたま商品名だけの行が出現した場合にも
誤って候補に含んでしまう可能性が残っていた。実機の再実行前にこの点を
再検討し、候補条件を「商品名の行の**直後**に『→ 商品を見る』の行が続く」
という隣接2行の完全一致に厳密化した。商品名の行だけがあり直後に「→ 商品
を見る」が続かないブロック(本文中のふつうの文章等)は候補にしない。
一意に特定できた場合も、そのブロック全体の行構成がちょうど2行である
ことを改めて確認する。あわせて、候補ブロックが1件以上見つかった場合
(一意特定・複数件での安全停止の両方)、そのブロックのindexと正規化済み
linesを診断ログへ記録するようにし、次回の実機実行で問題が起きた場合も
1回で原因を切り分けられるようにした(詳細は「10. 商品リンク」)。この
修正もローカルpytestでの確認のみで、まだ実際のGitHub Actions実行
(`ARTICLE-001`の再実行)では検証していない。

`ARTICLE-001`(direct child scopeへの限定)で判明した事項: 上記の隣接
2行判定への修正後も実機で「2件見つかった」事象が再現した。今回はユーザー
が実機のHTMLダンプを直接解析し、「TOY JAM 瀬戸内レモン→ 商品を見る」が
実際に2回入力されているわけではなく、本文editor配下に(1)本文・商品導線
などを内包している要素と、(2)実際の商品導線そのものの`<p>`、の両方が
存在し、`[label, "→ 商品を見る"]`という隣接2行が両方で(内包関係のまま)
成立していたことが真因だと判明した。`body_locator.locator("p")`は通常の
CSSセレクタとして本文editor配下の**あらゆる深さのdescendant**(子孫)の
`<p>`要素にマッチするため、direct childではない要素も候補に含めてしまって
いた。実機のdirect child構造を確認すると、商品導線は本文editorの直接の子
として独立した`<p>`で存在していたため、探索スコープを`body_locator.
locator(":scope > p")`(直接の子のみ)に限定した。隣接2行判定・行構成の
最終チェック・診断ログの仕組み自体は変更していない(詳細は「10. 商品
リンク」)。この修正もローカルpytestでの確認のみで、まだ実際のGitHub
Actions実行(`ARTICLE-001`の再実行)では検証していない。

`ARTICLE-001`(全文完全一致への厳密化)で判明した事項: direct child
scopeへの限定後、実機で`ARTICLE-001`を再実行したところ、再び商品名を
含むブロックが2件見つかる事象が再現した。今回はユーザーが実機のHTML
ダンプを直接解析し、本文editorの直接の子である`<p>`の中に、(A)記事本文
ほぼ全体を`inner_text()`として含む巨大な`<p>`と、(B)実際の商品導線専用の
`<p>`の両方が存在し、巨大な`<p>`(A)の`lines`の**途中**にも`[label,
"→ 商品を見る"]`という隣接2行が偶然含まれていたことが真因だと判明した。
「ブロック内のどこかに隣接2行が存在すれば候補」という従来の判定では
(A)(B)の両方が候補になっていたため、候補条件を「ブロックの`lines`全体が
ちょうど`[label, "→ 商品を見る"]`の2行だけで構成されているか」という
全文完全一致に厳密化した。探索スコープ(direct childのみ)・行構成の
最終チェック・診断ログの仕組み自体は変更していない(詳細は「10. 商品
リンク」)。この修正もローカルpytestでの確認のみで、まだ実際のGitHub
Actions実行(`ARTICLE-001`の再実行)では検証していない。
...
`ARTICLE-001`(commit d640224での再実行、運用方針変更)で判明した事項:
上記の`fill()`への変更を実機で再実行したところ、2商品目「TOY JAM 瀬戸内
レモン月桂樹」のURL入力・「適用」クリックまでは成功したが、
`_wait_for_product_link_applied()`が上限時間(3000ms)以内に対象ブロック
内への`<a>`要素の反映を確認できず、`NotePosterError`で`needs_review`へ
安全停止した(公開操作は行われていない)。これまでのARTICLE-001での一連の
実機実行(商品導線ブロックの誤判定、`LinkButtonOutOfViewportError`、URL
値の残留・連結、そして今回の反映timeout)を踏まえ、note.com側の商品
リンク設定UIは実機上で繰り返し不安定さが見られ、これ以上の自動化追跡は
コストに見合わないと判断した。

そのため運用方針を変更し、`product_links`が指定されている記事について
`create_draft()`が`_apply_product_links()`・`_assert_links_match()`を
呼ばないようにした。商品導線テキスト(商品名・「→ 商品を見る」)は本文へ
プレーンテキストとして自動生成するが、実際のリンク設定は人間が下書き上で
手動で行う。既存のstatus体系(ready/processing/draft_created/published/
error/needs_review)は変更せず、下書き作成自体が成功した場合は
`needs_review`に`note_url`を保持したまま倒すことで「下書き作成済み・
商品リンクの手動設定待ち」を表現する
(`StatusManager.mark_needs_review_with_note_url()`)。商品リンク自動
設定関連のメソッド・例外クラス自体は削除せず、将来note.com側のUIが
安定した場合の再検証に備えて残している(詳細は「10. 商品リンク」)。

`ARTICLE-001`は人間が確認するまで`status`を`ready`に戻さない
(対応済み・変更していない)。ARTICLE-001の実機再実行は今回も行っていない。
今回はARTICLE-001の既存note下書きを人間が確認し、2つの商品リンクを手動
設定する運用とする。
