...
**タグ確認の偽陽性について**: 実機テストで、入力したタグ(例:「自動投稿」)
が本文・タイトル中の同じ単語に誤って一致し、実際にはタグが反映されて
いないのに「確認できた」と誤判定する不具合が見つかりました。現在は
「#タグ名」という#付きの表示にのみ一致させ(#無しの素の単語には一致
させない)、全タグをまとめて確認したうえで欠けているものと実際に画面上に
見えている#付き文字列の一覧をエラーメッセージに含めるようにしています。
また、タグを1件確定させるごとにnote側の自動保存が完了するのを待ってから
次のタグへ進むようにし、保存の競合を避けています。
