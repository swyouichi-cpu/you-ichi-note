...
セクションを追記し、「→ 商品を見る」という固定文言だけに、noteの公式
リンク挿入ショートカット(`Control+K` / `Meta+K`、下記参照)経由で
インラインリンクを設定します。
...
**実機確認済み(2026年8月29日、人間による手動確認)**: note公式の
「エディタのガイド」に、リンク挿入のショートカット`⌘+K`(Mac)が
文書化されていることを確認しました。実機で、本文中の「→ 商品を見る」を
選択して`⌘+K`を押すとURL入力欄が開き、URLを設定すると選択した文字列
だけがインラインリンクになり(商品カードへは変換されない)、下書き保存後も
リンクが保持されることを確認しました。同ガイドには`⌘+S`(下書き保存)・
`Shift+Return`(段落内の改行、通常のEnterによる改段落とは別)も文書化
されています。

**リンク設定操作: note公式ショートカット(`Control+K` / `Meta+K`)**:
当初はリンクアイコンを含む選択ツールバーのボタンをクリックする方式で
実装していましたが、ボタンのDOM構造(アイコンのみでテキストラベルが
無く、role/ariaも未確認)への依存度が高く、noteのUI変更に弱いという懸念
がありました。上記の通り公式ドキュメントに存在が明記されているキーボード
ショートカットの方が安定した自動化の契約になると判断し、ボタンクリックを
撤去してショートカット方式に切り替えました(`_open_link_input_via_shortcut()`)。

GitHub Actionsのheadless ChromiumはLinux上で動作するため、noteのフロント
エンドJSが`navigator.platform`等でOS判定していた場合、Macの`⌘+K`ではなく
`Ctrl+K`を待ち受けている可能性があります(未確認だが十分にありうるリスク)。
そのため実装では`Control+K`→`Meta+K`の順に両方を試し、URL入力欄が実際に
出現したかをDOMで確認できた方を採用します。どちらでも入力欄の出現を確認
できない場合は、推測でクリックや入力を進めず`needs_review`へ安全停止します。

ショートカットを送る前に、対象の「→ 商品を見る」を`select_text()`で選択し、
`window.getSelection().toString()`を読み取って選択内容が期待通り
「→ 商品を見る」そのものであることを確認してから`Control+K`/`Meta+K`を
送信します(意図しない範囲を選択したままリンクを設定してしまう事故を防止)。
また、ショートカット送信後は`page.keyboard.press()`(要素へのフォーカス
移動や再検証を行わない、キーボードレベルの生イベント送信)を使うことで、
既に確定している選択状態を壊さないようにしています。
...
**リンク先の特定方法(`_resolve_link_target_for_label()`)**: 「→ 商品を見る」
は商品が複数ある場合、本文中に複数回出現します。以前はページ全体で
「→ 商品を見る」が何番目に出現するかという**フラットな出現順序
(`.nth(index)`)**で`product_links[N]`に対応付けていましたが、これは
本文が今後書き換わった場合などに意図しない出現と結びつくリスクがあるため、
より安全な方法に変更しました。

現在は、まず商品名(`label`)を`get_by_text(label, exact=True)`で本文中から
一意に(ちょうど1件)特定します。一意に特定できない場合(件数が0件または
2件以上)は`needs_review`へ安全停止します。次に、その商品名要素の**直後の
兄弟要素(`xpath=following-sibling::*[1]`)**を取得し、そのテキストが
「→ 商品を見る」と一致することを確認したうえで、初めてその要素をリンク
設定対象とします。一致しなければ(本文の構造が想定と異なる、商品名の直後に
別の要素がある等)、位置やインデックスの推測はせず`needs_review`へ安全停止
します。

これは`_fill_body()`で撤去した「画面上に見えるN番目のcontenteditable要素」
のような、DOM構造そのものを盲目的に推測する位置ベースフォールバックとは
明確に異なります。`following-sibling::*[1]`は「一意に特定できた商品名
要素」を起点にした**構造的な隣接関係**の参照であり、しかもその参照結果
(テキストが「→ 商品を見る」と一致するか)を必ず内容ベースで再確認して
いるため、起点の特定に失敗すればそこで安全停止し、隣接要素の内容が違えば
そこでも安全停止します。ページ全体でのN番目出現に依存する古い方式よりも、
対応関係の取り違えが起こりうる余地が小さくなっています。

なお、`_apply_product_links()`の冒頭では引き続き、本文中の「→ 商品を見る」
の出現総数が`product_links`の件数と一致するかという集計チェックも行って
います。件数が一致しない場合(人間が書いた本文に偶然同じ文言が含まれていた
等)は、対応関係の破綻を早期に検知するため、個別のラベル解決に進む前に
`needs_review`へ安全停止します。

**リンクURL入力欄のセレクタについて(未確定・要実機検証)**: 本文editorの
セレクタ(ProseMirror)とは異なり、`Control+K`/`Meta+K`送信後に現れる
URL入力欄の正確なDOM構造は、まだ実機のHTMLダンプで確認できていません
(人間による目視確認のみ)。そのため候補セレクタ(`input[type="url"]`、
`placeholder`に"URL"を含む、role="textbox"でnameに"URL"/"リンク"を含む等)
は一般的なリッチテキストエディタのリンク入力UIで使われがちな属性に基づく
best-effortなものです。候補が一致しない場合は位置ベースの推測に頼らず
`needs_review`へ安全停止します。実機で候補が一致しなかった場合は、本文
editorのセレクタを確定したときと同様、失敗時の診断データを元に実際の
DOM構造に基づいてセレクタを更新する想定です。**リンク設定操作を
ショートカット方式に切り替えたこの実装は、まだ実際のGitHub Actions実行
では検証していません**(人間による手動UI確認と、ローカルpytestでの
確認のみ)。次回の実機テストで検証します。
...
13. 商品リンクのリンク設定はnote公式ショートカット(`Control+K` /
    `Meta+K`)経由で行い、リンク設定UIを開くためのボタンクリックは行わない
    (ボタンクリックという操作自体が無くなったため、誤って別のボタンを
    クリックしてしまうリスクがそもそも存在しない設計)。ショートカット送信後
    に開くURL入力欄への`click()`は通常のテキスト入力欄へのフォーカス移動に
    過ぎないが、それでもURL入力欄を安全に特定できない場合は推測で操作せず
    `needs_review`。下書き保存ボタン(`_save_draft`)など、実際にボタンを
    クリックする箇所では引き続き直前に`_assert_not_publish_action()`を適用する
...
基づく`_resolve_link_target_for_label()`へ変更した。
**この実装(ショートカット方式・ラベル隣接方式とも)はまだ実際の
GitHub Actions実行では検証していない**(ローカルテストと、リンク設定が
可能であること自体の人間による手動UI確認のみで、自動化コードを実機で
動かした検証はまだ行っていない。特に`Control+K`と`Meta+K`のどちらが
実際に有効かは、次回のGitHub Actions実行で初めて確認できる)。
...
- 本文の段落内改行(`Shift+Return`)の自動化: note公式の「エディタの
  ...
  (同じ段落内の改行)が文書化されています。現在の本文入力
  (`press_sequentially()`)は、ドキュメント化されている通り本文中の`\n`を
  すべて`Enter`キー入力として扱うため、生成されるのは常に新しい段落であり、
  `Shift+Return`による段落内改行は使われていません。`TEST-003`では、
  本文とハッシュタグの間の5行の空行(`_TAG_SEPARATOR`)がnote上で正常に
  見た目通りに反映されることを実機確認済みです。人間が読みやすさのために
  意図的に空行を多用する記事フォーマット(段落間の「間」を作る書き方)も、
  現状は空行=空の段落として`Enter`の連続で表現されており、これ自体が
  ARTICLE-001等で問題を起こした事実は今のところありません。ただし、より
  noteの段落モデルに忠実な表現(同じ段落内の改行と、段落そのものの区切りを
  明確に書き分ける)をしたい場合は、`Shift+Return`をPlaywright側で明示的に
  送信する方式への変更が将来的な改善候補になります。これはPhase 1の要件
