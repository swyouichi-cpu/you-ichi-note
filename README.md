...
ショートカットを送る前に、上記の`_select_product_link_text_in_block()`で
対象の「→ 商品を見る」だけを選択し、`window.getSelection().toString()`を
読み取って選択内容が期待通り「→ 商品を見る」そのものであることを確認して
から`Control+K`/`Meta+K`を送信します(意図しない範囲を選択したままリンクを
設定してしまう事故を防止)。
